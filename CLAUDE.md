...
フェーズを遡る必要が生じた場合（例: /finetune 中に /data の問題が発覚）は、ユーザーに報告した上で前フェーズに戻ること。
...
#### 精度目標（/ideate で設定、/finetune の完了条件に反映すること）
...
### 4. 本格 FineTune・検証 (/finetune)
【入場条件】/poc2 完了（LoRA FT 有効性・汎化特性の確認済み）
【完了条件】モデルが動作し、/poc2 との定量比較（Precision/Recall/F1 + **False Positive Rate**）が完了し、lean_canvas.md の Key Metrics 目標値を達成していること
- **データ収集**: FIRMS POS を ~100 件規模に拡充 + 火災非経験地域の多様な NEG を追加 (/poc2 汎化確認で FP が出やすかったカテゴリを優先)
- **本格 LoRA FT**: /poc2 の 20 サンプルを数百件規模に拡張して再実行（デフォルト epochs=5）
- **評価**: FIRMS 地点テストセット + 汎化テストセット (非 FIRMS 地域) で別々に Recall/FP Rate を計測し、/poc2 実績値との差分を示す
- **RTX 5090 でのパフォーマンス計測** (VRAM・レイテンシ)
- LoRA 重みと学習コードの公開（方法は提出方法確定後に決定）

### 5. 提出準備 (/docs)
【入場条件】/finetune 完了 かつ アプリが審査員によるデバッグなしで動作すること
