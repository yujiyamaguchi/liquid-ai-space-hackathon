...
SIZE_KM         = 5.0         # シーンサイズ
# NEG FIRMS照合の検索半径: シーン半径 (SIZE_KM/2) に煙ドリフト等のバッファとして
# シーン1辺分を加算。SIZE_KM 変更時は自動追随する。
# 例: SIZE_KM=5.0 → half=2.5km + 5.0km buffer = 7.5km ≈ 0.068°
NEG_FIRMS_RADIUS_DEG = round((SIZE_KM / 2 + SIZE_KM) / 111.0, 4)  # ≈ 0.068°
...
def _find_fire_free_neg_candidates(event: dict) -> list[str]:
    ...
    for offset_days in NEG_OFFSET_CANDIDATES:
        ...
        fires = fetch_firms_at_date(lon, lat, neg_date,
                                    radius_deg=NEG_FIRMS_RADIUS_DEG,
...
def _event_to_pos_ts(event: dict, shift_days: int = SHIFT_DAYS) -> str:
    """FIRMS イベント → POS タイムスタンプ (検知日時 + shift_days)

    acq_time (HHMM UTC) を使い検知時刻を反映する。
    SimSat は後方サーチなので timestamp が天井になり、
    検知時刻以降の最新シーンが返ってくる確率が上がる。
    """
    time_str = str(event.get("time", "1200")).zfill(4)
    hh, mm = int(time_str[:2]), int(time_str[2:])
    dt = datetime.strptime(event["date"], "%Y-%m-%d").replace(
...
class DatasetBuilder:
    ...
    def collect_firms(self, n_pos: int = 100, min_frp: float = 0.0,
        ...
        for ev in events:
            ...
            # 重複チェック用: 最初の試行日付 (SHIFT_DAYS=2) で判定
            dedup_date = _event_to_pos_ts(ev, shift_days=SHIFT_DAYS)[:10]

            # 同一座標・同一日付の重複イベントはスキップ (カウントしない)
            # ※ FIRMS は同一火災を複数ピクセルで報告するため重複が多い
            if self._already_fetched(lon, lat, dedup_date):
                continue

            # --- POS: リトライ付き取得 ---
            # SimSat は後方サーチ。SHIFT_DAYS を段階的に増やし、
            # FIRMS 検知時刻以降のシーンが返るまで最大3回試みる。
            time_str = str(ev.get("time", "1200")).zfill(4)
            firms_dt_str = f"{ev['date']}T{time_str[:2]}:{time_str[2:]}:00Z"
            desc_pos = f"FIRMS FRP={ev['frp']:.0f}MW ({lon:.2f},{lat:.2f}) {ev['date']}"

            pos_result = None
            ts_date_pos = dedup_date  # 成功時に上書き
            for attempt, extra_days in enumerate([0, 3, 7]):
                shift = SHIFT_DAYS + extra_days
                pos_ts_try = _event_to_pos_ts(ev, shift_days=shift)
                suffix = f" (retry+{extra_days}d)" if attempt > 0 else ""
                print(f"  POS {desc_pos}{suffix} ...", end=" ", flush=True)
                t0 = time.perf_counter()
                result = self._fetch_scene(lon, lat, pos_ts_try)
                if result is None:
                    break  # cloud/quality → リトライ不要 (メッセージは _fetch_scene が出力済み)
                # Δ≥0: capture datetime が FIRMS 検知時刻以降か確認 (datetime 精度)
                if result["capture"] < firms_dt_str:
                    print(f" → Δ<0 (capture={result['capture'][:16]} < firms={firms_dt_str[:16]})", end="", flush=True)
                    continue  # Δ<0 のみリトライ
                pos_result = result
                ts_date_pos = pos_ts_try[:10]
                print(f"✅ NBR2_min={result['scene'].indices.nbr2_min:.3f} ({time.perf_counter()-t0:.1f}s)")
                break
            else:
                print(f" → 全リトライで Δ<0 → スキップ")
