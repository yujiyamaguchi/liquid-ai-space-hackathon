...
| **オンボードストレージ** | 1 GB（1ヶ月） | Winner Prize の in-space storage から推定 |
...
**1 GB ストレージの戦略的活用**: 1GB は「オンボードAIをステートフルにする」ための資源として使う。地上処理システムは毎回ゼロから始めるが、オンボードキャッシュにより前回パスのデータと即時比較が可能になる。これがハッカソンの "What if satellites had onboard compute?" 与件の核心的な価値のひとつ。
...
**合計見積**: 33（AOIベースライン）+ 18（Few-shot）+ 10（アラートキュー）+ 13（LFM入力バッファ）+ 時系列統計（シーン数による）≈ **80〜200 MB**。1 GB に対して余裕あり。
